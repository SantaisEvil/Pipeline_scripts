...
for (i in 1:length(files)){
  ...
  if (filename != "ucsc" & filename != 'normalization.txt' & filename != 'normalized_files' & filename!='ucsc_norm_2' & filename!='alignment_stats.csv') {
    ...
    data <- c(sample_name,seq_depth,mapped_frags,alignment_rate, read_pair_dups,percent_dup)
...
colnames(df) <- c("Sample Name","Sequencing Depth","Mapped Fragments", "Alignment Rate", "Read Pair Duplicates", "Percent Duplication")
